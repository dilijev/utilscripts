...
(function () {
  ...
  async function run() {
    ...
    if (!container) {
      console.warn(`${LOG} #chat-scroll-container not found`);
      return;
    }
    const msgEls = container.querySelectorAll(`[${ATTR}]`);
    console.log(`${LOG} Visible chat messages in #chat-scroll-container: ${msgEls.length}`);
    let injected = 0;
    let missing = 0;
    let logOutput = [];
    for (const el of msgEls) {
      const id = el.getAttribute(ATTR);
      const dbInfo = map.get(id);
      logOutput.push(`${id}: ${dbInfo ? fmt(dbInfo) : 'NO TIMESTAMP'}`);
      if (dbInfo) {
        injectStamp(el, dbInfo);
        injected++;
      } else {
    ...
    console.log(`${LOG} Visible message timestamps:`);
    logOutput.forEach(line => console.log(`  ${line}`));
    console.log(`${LOG} run() done — injected: ${injected}, missing: ${missing}`);
  }
